...
分析範囲は従来の手動markerに加え、`=== エリア名 ===`形式のログから生成したエリアログ区間でも指定できる。エリアログ区間は時刻ではなくcanonicalのorderで区切り、エリアチェンジ行の直後から次のエリアチェンジ行の直前までを対象とする。同名エリアは訪問回数、通番、order範囲、ログ件数で区別する。

### リアルタイム分析

- 「分析開始」以後のcanonicalレコードを対象とする。
- 「分析終了」時点で範囲を固定する。
- 「PTメンバー設定...」で最大6名を登録し、ログから検出したPC候補からも追加できる。
- PTメンバー設定は保存され、オーバーレイへ即時反映される。
